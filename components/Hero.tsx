...
const Hero = ({ mainData }: HeroProps) => {
    ...
    const name = personalData?.name || mainData?.name
    const heroImage = personalData?.profile || mainData?.heroImage
    const shortDesc = personalData?.description || mainData?.shortDesc
    ...
    const techStackImages = mainData?.techStackImages || [
        "https://img.icons8.com/color/144/null/flutter.png",
        "https://img.icons8.com/color/144/null/dart.png",
        "https://img.icons8.com/color/144/null/nodejs.png",
        "https://img.icons8.com/color/144/null/firebase.png"
    ]
        ...
        <section id='home' className={`${theme === 'dark' && "bg-grey-900"} relative min-h-screen w-full mx-auto overflow-hidden`}>
            ...
            <div className="py-16 lg:py-48 flex flex-col-reverse lg:flex-row justify-around gap-10 lg:gap-0">
                ...
                <div className="flex flex-col gap-4 md:gap-6 text-left lg:w-1/2 2xl:w-1/3 mx-4 md:mx-6 xl:mx-0">
                    <div className="flex items-center gap-2 w-fit px-4 py-1.5 rounded-full border border-gray-300 dark:border-gray-700 bg-gray-50 dark:bg-[#0d1224] shadow-sm mb-4">
                    ...
                    <div className="flex items-center gap-3 flex-wrap pt-2">
                        <ScrollLink
                            className="w-fit text-sm md:text-base py-2.5 px-5 cursor-pointer flex items-center gap-1.5 rounded-md bg-violet-600 hover:bg-violet-700 dark:bg-violet-700 hover:dark:bg-violet-800 transition-colors group text-white font-medium"
                            to={'about'}
                            offset={-60}
                            smooth={true}
                            duration={500}
                        ...
                        <a
                            href={personalData?.resume || "/Muhammad_Abdul_Rehman_Flutter-ats.pdf"}
                            target="_blank"
                            rel="noopener noreferrer"
                            download="Muhammad_Abdul_Rehman_Flutter-ats.pdf"
                            className="w-fit text-sm md:text-base py-2.5 px-5 cursor-pointer flex items-center gap-2 rounded-md border-2 border-violet-600 text-violet-600 dark:text-violet-400 hover:bg-violet-600 hover:text-white dark:hover:bg-violet-600 dark:hover:text-white transition-all font-medium"
                        >
                ...
                <div className="relative mx-auto lg:mx-0 mt-12 md:mt-16 lg:mt-0">
                    <div className="w-56 h-56 md:w-80 md:h-80 lg:-translate-x-16">
                        <Image alt='avatar' width={1000} height={1000} className="rounded-full w-full h-full object-cover shadow-2xl" src={heroImage} priority />
                    </div>
