...
const About = ({ aboutData, name: propName }: Props) => {

    const aboutImage = personalData?.profile || aboutData?.aboutImage;
    const aboutImageCaption = aboutData?.aboutImageCaption;
    const title = personalData?.designation || aboutData?.title;
    const about = personalData?.description || aboutData?.about;
    const resumeUrl = personalData?.resume || aboutData?.resumeUrl || "";
    const callUrl = aboutData?.callUrl || "";
    const location = personalData?.address || aboutData?.location;
    const name = personalData?.name || propName;
        ...
        <SectionWrapper id="about" className="min-h-[90vh] pt-12 bg-gradient-to-b from-white to-gray-100/20 dark:from-grey-900 dark:to-grey-900">
            ...
            <div className="w-full lg:w-11/12 2xl:w-4/5 mt-2 lg:mt-20 mx-auto flex flex-col md:gap-4 lg:flex-row justify-between items-center">
                <div className="p-3 w-56 self-start md:w-2/5 lg:w-72 bg-white dark:bg-grey-800 flex flex-col gap-2 items-center rounded-2xl mx-auto lg:mx-16 hover:-translate-y-2 transition-transform duration-300 lg:-rotate-3">
                    ...
                    <span className="font-medium font-sans text-sm md:text-base">{aboutImageCaption || '< Software Engineer />'}</span>
                </div>
                ...
                <div className="flex-1 text-left mx-4 mt-4 md:mt-0 md:mx-0 md:p-6">
                    <div className="flex flex-col gap-2.5">
                        ...
                        <div className="flex flex-wrap items-center gap-2">
                            <p className='text-violet-800 w-fit rounded py-1 px-2 text-sm dark:text-violet-400 bg-violet-50 dark:bg-violet-900/20 font-medium'>{title}</p>
                            {location && (
                                <span className='text-gray-600 dark:text-gray-400 text-sm flex items-center gap-1.5 font-medium'>
                                    <FaMapMarkerAlt className="text-violet-600 text-xs" />
                                    <span>{location}</span>
                                </span>
                            )}
                        </div>
                        <p className="text-sm md:text-base my-2 text-gray-600 dark:text-gray-300 leading-relaxed">{about}</p>
